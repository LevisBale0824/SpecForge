// ---------------------------------------------------------------------------
// Render Worker — Full Shiki + MarkdownIt + Myers Diff Engine
// ---------------------------------------------------------------------------
// Web Worker for off-main-thread code rendering.
// Supports: Shiki syntax highlighting, MarkdownIt rendering, Myers diff,
//           grep pattern matching, line numbers (single/double gutter).
// Ported from xenodrive/vis (opencode-visualizer-cn).
// ---------------------------------------------------------------------------
...
type MarkdownShikiOptions = MarkdownItShikiSetupOptions & {
...
const HEX_COLOR_REF_RE =
  /^#(?:[0-9a-f]{3}|[0-9a-f]{4}|[0-9a-f]{6}|[0-9a-f]{8})$/i;
const FUNC_COLOR_REF_RE =
  /^(?:rgb|rgba|hsl|hsla)\(\s*[\d.]+%?\s*(?:[,/]\s*[\d.]+%?\s*)*\)$/i;

function isInlineColorRef(value: string) {
  return HEX_COLOR_REF_RE.test(value) || FUNC_COLOR_REF_RE.test(value);
}

function parseInlineFileRef(
  rawRef: string,
  fileSet: Set<string>,
): ParsedInlineFileRef | null {
...
function wrapDiffRows(
  ...
  return lines
    .map((row, index) => {
      ...
      if (mode === "none")
        return `<div class="${rowClass.trim()}">${row.html}</div>`;
      if (mode === "single") {
...
function reconstructSourcesFromDiff(diff: string): {
  before: string;
  after: string;
} {
...
function generateUnifiedDiff(
  ...
  for (let d = trace.length - 1; d >= 0; d--) {
    ...
    if (
      k === -d ||
      (k !== d && snap[offset + k - 1] < snap[offset + k + 1])
    ) {
      prevK = k + 1;
    } else {
      prevK = k - 1;
...
function applyPatchToCode(code: string, patch: string) {
  ...
  while (index < patchLines.length) {
    ...
    let pointer = oldLine - 1 + patchOffset;
    index += 1;
    while (
...
function highlightGrepMatches(line: string, matcher: RegExp | null) {
  ...
  while (cursor <= line.length) {
    ...
    if (matchIndex > cursor) {
      html += escapeHtml(line.slice(cursor, matchIndex));
    }
    if (!value) {
      if (matchIndex >= line.length) break;
      html += escapeHtml(line[matchIndex] ?? "");
      cursor = matchIndex + 1;
    ...
    if (matcher.lastIndex <= matchIndex)
      matcher.lastIndex = matchIndex + value.length;
  }
...
function renderGrepRows(
  ...
  return lines
    .map((line, index) => {
      ...
      if (mode === "none")
        return `<div class="code-row">${content}</div>`;
      if (mode === "double") {
        const pair = gutterLines?.[index]?.split("\t") ?? [];
        const left = pair[0] ?? "";
        const right = pair[1] ?? "";
...
function buildDiffHtmlFromCode(
  ...
  return getHighlighter(theme).then(async (highlighter) => {
    ...
    const trimmedBefore =
      maxOld > 0
        ? effectiveBefore.split("\n").slice(0, maxOld).join("\n")
        : effectiveBefore;
    const trimmedAfter =
      maxNew > 0
        ? effectiveAfter.split("\n").slice(0, maxNew).join("\n")
        : effectiveAfter;

    const beforeHtml = safeCodeToHtml(
      highlighter,
      trimmedBefore,
      resolvedLang,
      theme,
    );
    const afterHtml = safeCodeToHtml(
      highlighter,
      trimmedAfter,
      resolvedLang,
      theme,
    ...
    diffLines.forEach((line) => {
      ...
      if (line.startsWith("+") && !line.startsWith("+++")) {
        const htmlLine =
          afterLines[newLine - 1] ??
          `<span class="line">${escapeHtml(line.slice(1))}</span>`;
...
async function renderCodeHtml(request: RenderRequest) {
  ...
  if (hasRange) {
    const offset =
      typeof request.lineOffset === "number"
        ? Math.max(0, request.lineOffset)
        : 0;
    const limit =
...
function getMarkdownIt(highlighter: Highlighter, theme: string) {
  if (
    ...
    cachedMd.renderer.rules.link_open = function (
      tokens,
      idx,
      options,
      _env,
    ...
    cachedMd.renderer.rules.fence = function (
      tokens,
      idx,
      options,
      _env,
    ...
    cachedMd.renderer.rules.code_inline = function (
      tokens,
      idx,
      options,
      env,
      ...
      const parsed =
        fileSet && ref ? parseInlineFileRef(ref, fileSet) : null;
      if (parsed) {
        ...
        token.attrSet(
          "style",
          existingStyle
            ? `${existingStyle}; --preview-color: ${ref}`
            : `--preview-color: ${ref}`,
        );
      }
...
function renderRequest(request: RenderRequest): Promise<string> {
  if (request.patch) {
    const after =
      request.after ?? applyPatchToCode(request.code, request.patch);
