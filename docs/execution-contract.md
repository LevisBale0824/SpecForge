...
| `requirements` | delta specs 的 `### Requirement:` 头 + `#### Scenario:` | 每个 SHALL/MUST(及其 scenario)都必须有对应 completed task |
...
### 4.3 覆盖率的两层追溯：Requirement 级 + Scenario 级

`checkRequirementsCoverage` 的判据分两层：

**Requirement 级（基础）**：契约里每个 `requirement.name` 必须能在 `change.tasks`
里找到一条 `status === "completed" && requirement === name` 的 task。这是硬性的，
任何 SHALL/MUST 缺绑定即 NOT_READY。

**Scenario 级（升级）**：若该 requirement 在契约里列出了 `scenarios[]`，且 tasks.md 中
**至少有一个**任务显式绑定了 `- Scenario: <name>`，则该 requirement 进入**严格模式**——
每个 scenario 都须有至少一个 completed 任务经 `- Scenario: <name>` 显式绑定。

**宽松回退**：若某 requirement 的所有任务都**没有**显式 Scenario 绑定（仅绑了 Requirement），
视为隐式覆盖全部 scenario。这样兼容存量 tasks.md；但一旦 LLM 开始用 Scenario 绑定，
就必须全覆盖，否则 Verify 会精确报出漏绑的 scenario 名。

**没有**做更复杂的语义匹配（比如解析 task 描述里的关键词、跑测试反向映射等），理由：

- 显式绑定（`task.requirement` / `task.scenario`）比启发式匹配更可靠
...
| Verify Pre-Gate 显示"scenario 未覆盖"    | tasks.md 对该 requirement 用了 Scenario 绑定，但漏了某几个 | 补 `- Scenario: <Name>` 绑定，或回退为仅绑 Requirement（隐式覆盖全部） |
