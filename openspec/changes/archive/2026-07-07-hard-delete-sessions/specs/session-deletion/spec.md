...
定义 SpecForge 中"删除会话"的可验收契约：删除须是**持久化的硬删除**（从后端/磁盘抹除，而非前端隐藏），须**级联**删除全部子线程，须有**确认门**拦截误删，且失败须**显形**而非静默吞错。本 capability 覆盖 OpenCode 与 Zero 两套后端 adapter 的 `deleteSession` 路径。
